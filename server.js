...
admin.initializeApp({
  ...
  databaseURL: "https://myteams-af8ac-default-rtdb.firebaseio.com"
});
...
let API_KEY_SECRET = process.env.API_KEY_SECRET || "WalkTalk_default_secret";
...
app.post(["/api/v1/meeting"], (req, res) => {
  ...
  if (authorization != API_KEY_SECRET) {
    logme("WalkTalk get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
    return res.status(403).json({ error: "Unauthorized!" });
  }
  // setup WalkTalk meeting URL
  ...
  logme("WalkTalk get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
